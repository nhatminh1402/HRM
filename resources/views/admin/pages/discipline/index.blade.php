...
@section('content')
    ...
    <form action="" method="POST" class="mb-4">
        ...
        <button class="btn btn-success">
            <svg class="mb-1 me-2" xmlns="http://www.w3.org/2000/svg" height="16" width="14" viewBox="0 0 448 512">
                <path
                    d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z"
                    fill="white" />
            </svg>Thêm loại kỷ luật</button>
    </form>
    <div class="d-flex justify-content-between w-100 flex-wrap">
        <div class="mb-3 mb-lg-0">
            <h3 class="h3">Danh sách loại kỷ luật </h3>
        </div>
                    ...
                    @else
                        @foreach ($disciplines as $key => $discipline)
                            <tr>
                                ...
                                <td class='text-center'>{{ $discipline->description }}</td>
                                <td class='text-center'>{{ $discipline->created_at }}</td>
                                <td class='text-center'>{{ $discipline->updated_at }}</td>
                                <td class='text-center'>
                                    <a class="btn btn-warning" href="{{ route('admin.discipline.edit', ['id' => $discipline->id, 'page' => $pageNumber]) }}">
                                ...
                                <td class='text-center'>
                                    <form id="form-delete" {{-- action="{{ route('admin.employee.position.destroy', $position->id) }}" --}} method="POST">
                                        @csrf
                                        @method('DELETE')
